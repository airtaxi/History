...
public sealed partial class BookmarkedPostsPage : BasePage, IRecipient<RefreshRequestedMessage>
{
    protected override BaseTimelinePageViewModel ViewModel { get; }

    public BookmarkedPostsPage()
    {
        ViewModel = App.Services.GetRequiredService<BookmarkedPostsPageViewModel>();
    ...
    // ScrollingScrollOptions with AnimationMode.Disabled pre-realizes the target range through
    // the ScrollPresenter's anticipated viewport path before the compositor commits the new offset.
