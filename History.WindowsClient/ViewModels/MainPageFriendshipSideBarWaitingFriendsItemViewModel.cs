...
public partial class MainPageFriendshipSideBarWaitingFriendsItemViewModel : BaseMainPageFriendshipSideBarItemViewModel
{
    ...
    public MainPageFriendshipSideBarWaitingFriendsItemViewModel(MainPageFriendshipSideBarViewModel parentViewModel) : base(parentViewModel)
    {
    ...
    public override async Task RefreshAsync()
    {
        if (!Parent.Parent.IsKakaoStoryMode)
        {
            ...
            if (!result.IsSuccess)
            {
                await Parent.Parent.ShowMessageDialogAsync(new("오류", "보낸 친구 신청 목록을 가져오는 데에 실패하였습니다."));
                return;
            }
