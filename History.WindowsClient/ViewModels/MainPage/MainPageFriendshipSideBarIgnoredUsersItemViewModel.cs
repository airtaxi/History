...
public partial class MainPageFriendshipSideBarIgnoredUsersItemViewModel : BaseMainPageFriendshipSideBarItemViewModel, IRecipient<FriendshipChangedMessage>
{
    ...
    public MainPageFriendshipSideBarIgnoredUsersItemViewModel(MainPageViewModel hostViewModel) : base(hostViewModel)
    {
    ...
    public override async Task RefreshAsync()
    {
        if (!HostViewModel.IsKakaoStoryMode)
        {
            ...
            if (!result.IsSuccess)
            {
                await HostViewModel.ShowMessageDialogAsync(new("오류", "무시한 사용자 목록을 가져오는 데에 실패하였습니다."));
                return;
            }
