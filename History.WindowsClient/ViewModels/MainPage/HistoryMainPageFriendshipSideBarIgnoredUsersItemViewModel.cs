...
public partial class HistoryMainPageFriendshipSideBarIgnoredUsersItemViewModel : BaseMainPageFriendshipSideBarItemViewModel, IRecipient<FriendshipChangedMessage>
{
    private ObservableCollection<BaseFriendshipViewModel> _items;

    public HistoryMainPageFriendshipSideBarIgnoredUsersItemViewModel(MainPageViewModel baseViewModel) : base(baseViewModel)
    {
    ...
    {
        var result = await BaseViewModel.ExecuteRequestAsync(new GetIgnoredUsers());
        if (!result.IsSuccess)
        {
            await BaseViewModel.ShowMessageDialogAsync(new(Constants.ErrorTitle, "무시한 사용자 목록을 가져오는 데에 실패하였습니다."));
            return;
        }
