...
public partial class HistoryMainPageFriendshipSideBarWaitingFriendsItemViewModel : BaseMainPageFriendshipSideBarItemViewModel, IRecipient<FriendshipChangedMessage>
{
    private ObservableCollection<BaseFriendshipViewModel> _items;

    public HistoryMainPageFriendshipSideBarWaitingFriendsItemViewModel(MainPageViewModel baseViewModel) : base(baseViewModel)
    {
    ...
    {
        var result = await BaseViewModel.ExecuteRequestAsync(new GetWaitingRequests());
        if (!result.IsSuccess)
        {
