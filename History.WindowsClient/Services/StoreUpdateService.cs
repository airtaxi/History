...
public sealed class StoreUpdateService(ApplicationSettingsService settingsService, ApplicationNotificationService applicationNotificationService) : IDisposable
{
    ...
    public void Dispose()
    {
        ...
        if (_isStarted) settingsService.Settings.PropertyChanged -= OnApplicationSettingsPropertyChanged;
        StopMonitoring();
    }
    ...
    {
        if (settingsService.Settings.IsAutomaticUpdateCheckEnabled) StartMonitoring();
        else StopMonitoring();
    }
