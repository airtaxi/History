...
// Local copy of the CommunityToolkit StaggeredLayout (MIT) with the manual element
// recycling in MeasureOverride removed: the ItemsRepeater auto-recycles elements a
// layout pass does not touch, and recycling mid-pass leaves elements in the recycle
// pool while they are still attached, corrupting the realization state during page
// navigation ("Element is already the child of another element" / "ItemsRepeater's
// child not found in its Children collection").
...
namespace History.WindowsClient.ThirdParty.StaggeredLayout;
...
public partial class StaggeredLayout : VirtualizingLayout
{
    ...
    public static readonly DependencyProperty ItemsStretchProperty = DependencyProperty.Register(nameof(ItemsStretch), typeof(StaggeredLayoutItemsStretch), typeof(StaggeredLayout), new PropertyMetadata(StaggeredLayoutItemsStretch.None, OnItemsStretchChanged));
    ...
    /// </summary>
    public static readonly DependencyProperty ColumnSpacingProperty = DependencyProperty.Register(nameof(ColumnSpacing), typeof(double), typeof(StaggeredLayout), new PropertyMetadata(0d, OnSpacingChanged));
    ...
    /// </summary>
    public static readonly DependencyProperty RowSpacingProperty = DependencyProperty.Register(nameof(RowSpacing), typeof(double), typeof(StaggeredLayout), new PropertyMetadata(0d, OnSpacingChanged));
    ...
    protected override Size MeasureOverride(VirtualizingLayoutContext context, Size availableSize)
    {
        ...
        if (context.RealizationRect is { Width: 0, Height: 0 }) return new Size(availableSize.Width, 0.0f);
        ...
        if (Math.Abs(columnWidth - state.ColumnWidth) > double.Epsilon) state.Clear();

        state.ColumnWidth = columnWidth;

        // adjust for column spacing on all columns expect the first
        double totalWidth = state.ColumnWidth + ((numColumns - 1) * (state.ColumnWidth + ColumnSpacing));
        if (totalWidth > availableWidth) numColumns--;
        else if (double.IsInfinity(availableWidth)) availableWidth = totalWidth;

        // The items will not need to be remeasured, but they will need to go into new columns
        if (numColumns != state.NumberOfColumns) state.ClearColumns();
        ...
        for (int i = 0; i < context.ItemCount; i++)
        {
            ...
            {
                // No manual recycling here: the ItemsRepeater auto-recycles elements a
                // layout pass does not touch, and recycling mid-pass would let the same
                // pass re-realize an element that is still attached, corrupting the
                // realization state during page navigation.
            }
            ...
            if (deadColumns.Count == numColumns) break;
        }
    ...
    {
        if ((context.RealizationRect.Width == 0) && (context.RealizationRect.Height == 0)) return finalSize;
        ...
        for (int columnIndex = 0; columnIndex < state.NumberOfColumns; columnIndex++)
        {
            ...
            for (int i = 0; i < layout.Count; i++)
            {
                ...
                if (bottom < context.RealizationRect.Top) continue;
