...
internal class StaggeredLayoutState
{
    ...
    internal void AddItemToColumn(StaggeredItem item, int columnIndex)
    {
        ...
        if (columnLayout.Contains(item) == false) columnLayout.Add(item);
    }
    ...
    internal double GetHeight()
    {
        ...
        if (itemCount == _context.ItemCount) return desiredHeight;

        double averageHeight = 0;
        foreach (var kvp in _columnLayout) averageHeight += kvp.Value.Height / kvp.Value.Count;

        averageHeight /= _columnLayout.Count;
        double estimatedHeight = (averageHeight * _context.ItemCount) / _columnLayout.Count;
        if (estimatedHeight > desiredHeight) desiredHeight = estimatedHeight;

        if (Math.Abs(desiredHeight - _lastAverageHeight) < 5) return _lastAverageHeight;
