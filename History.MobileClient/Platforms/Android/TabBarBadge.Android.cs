...
public static partial class TabBarBadge
{
        ...
        double fontSize)
    {
        ...
        if (tabIndex < 0 || tabIndex >= bottomNav.Menu.Size()) return;

        var itemContainer = FindTabItemContainer(bottomNav);
        if (itemContainer == null || tabIndex >= itemContainer.ChildCount) return;
        ...
        if (existing != null) tabButton.RemoveView(existing); // ensures anchor updates
        ...
        if (isDot) text = "●"; // could also use "•" for smaller dot
        ...
        if (!string.IsNullOrWhiteSpace(text))
        {
            ...
            else
            {
                ...
                if (isDot) label.SetTextColor(color.ToPlatform());
            }
    ...
    static partial void HideImpl(int tabIndex)
    {
        ...
        var tag = 90000 + tabIndex;
        var existing = tabButton.FindViewWithTag(tag);
        if (existing != null) tabButton.RemoveView(existing);
    }
    ...
    static string ForceTextGlyph(string s)
    {
        ...
        if (lastCode == 0xFE0E || lastCode == 0xFE0F) return s; // already has a selector
