...
public class CustomTabBarAppearanceTracker : ShellTabBarAppearanceTracker
{
    ...
    private static readonly Dictionary<int, BadgeState> s_badgeStates = new();
    ...
    public override void UpdateLayout(UITabBarController controller)
    {
        ...
        foreach (var index in s_badgeStates.Keys.ToList()) ApplyStoredBadge(index);
    }
        ...
        double fontSize)
    {
        ...
        {
            s_badgeStates.Remove(tabIndex);
            RemoveBadge(tabIndex);
            return;
        }

        // Store badge state so it survives icon rebuilds
        s_badgeStates[tabIndex] = new BadgeState
        {
            IsDot = isDot,
            Text = text,
            Bg = bg,
    ...
    private static void ApplyStoredBadge(int tabIndex)
    {
        var tabCount = s_controller?.TabBar?.Items?.Length ?? 0;
        if (tabIndex < 0 || tabIndex >= tabCount)
        {
        ...
        if (!s_badgeStates.TryGetValue(tabIndex, out var state)) return;
    ...
    private static void RemoveBadge(int tabIndex)
    {
        DispatchQueue.MainQueue.DispatchAsync(() =>
        {
    ...
    private static UIView GetTabButton(int tabIndex, bool getActive)
    {
        ...
        if (tabCount == 0 || total == 0) return null;
    ...
    private static UIView PickLayer(UIView[] layers, bool getActive, int tabIndex)
    {
    ...
    private static bool IsActiveTab(int tabIndex)
    {
    ...
    private static IEnumerable<UIView> FindLogicalTabButtons(UIView root)
    {
        foreach (var v in root.Subviews)
        {
            if (v is UIControl)
            {
                ...
                if (hasImage) yield return v;
            }

            foreach (var child in FindLogicalTabButtons(v)) yield return child;
        }
    }

    private static void ApplyBadgeToLayer(UIView button, int tabIndex, BadgeState state)
    {
    ...
    private static void ApplyPositionConstraints(
        UIView button,
        UIView badge,
        UIView refView,
        int anchorX,
