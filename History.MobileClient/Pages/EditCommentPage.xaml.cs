...
public partial class EditCommentPage : ContentPage
{
    ...
    private async Task EditKakaoCommentAsync()
    {
        ...
        try
        {
            ...
            if (payload == null)
            {
                await DisplayAlertAsync("오류", "첨부한 이미지(webp, heic, heif, avif)를 png로 변환하는 데 실패하여 댓글을 수정할 수 없습니다. 일반적으로 애니메이션이 포함된 webp 이미지이거나 기기에서 지원되지 않는 형식입니다.", Constants.PromptOk);
                return;
            }
