...
- **WebView hibernation common**: All Blazor pages receive `BlazorWebViewHibernationMessage` (bool) via `WeakReferenceMessenger`. When the app goes to background (`App.OnWindowStopped`) they call Android `WebView.OnPause()` + `PauseTimers()` + `Visibility.Gone`; on foreground return (`OnWindowResumed`) they restore in the order `Visibility.Visible` → `ResumeTimers()` → `OnResume()`. OnPause stops rendering (animations/video), PauseTimers stops JS timers/WebSockets, and GONE removes the view from layout/draw passes — so background Blazor tabs burn no CPU even when the realtime notification FGS keeps the process alive.
