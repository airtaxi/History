...
import type { PostResponseDto, UserResponseDto, PostReaction } from '@/types';
...
export function useReactions(post: PostResponseDto) {
  ...
  const reactionUsersMap: Ref<Record<string, UserResponseDto[]>> = ref({});
  ...
  const loadReactionData = async () => {
    try {
      ...
      const usersMap: Record<string, UserResponseDto[]> = {};
      let currentUserReaction: string | null = null;

      postReactions.forEach((reaction: PostReaction) => {
