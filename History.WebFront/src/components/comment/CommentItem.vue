...
const saveComment = async () => {
  ...
  const contents: any[] = []; // eslint-disable-line @typescript-eslint/no-explicit-any
...
const navigateToProfile = async (mentionText: string) => {
  ...
  try {
    ...
    const user = response.data.find((u: any) => u.nickname === nickname); // eslint-disable-line @typescript-eslint/no-explicit-any
    if (user) {
