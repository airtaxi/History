...
import PostAdvancedOptions from './PostAdvancedOptions.vue';
...
/**
 * 리포스트할 원본 게시글 정보
 * @type {import('vue').ComputedRef<any>} // eslint-disable-line @typescript-eslint/no-explicit-any
 */
...
const submitPost = async () => {
  ...
  try {
    ...
    const postDto = {
      ...
      Contents: [] as any[], // eslint-disable-line @typescript-eslint/no-explicit-any
    ...
    if (textWithoutHashtags) {
      ...
      const textParts: Array<any> = []; // eslint-disable-line @typescript-eslint/no-explicit-any
