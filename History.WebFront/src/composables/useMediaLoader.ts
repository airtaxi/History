...
export function useMediaLoader() {
   ...
   */
  const getMediaBlobUrl = async (mediaId: string, isProfileImage: boolean = false): Promise<string> => {
    const targetMap = isProfileImage ? profileBlobUrlMap.value : mediaUrlMap.value;
    if (targetMap[mediaId]) {
      return targetMap[mediaId];
    }
