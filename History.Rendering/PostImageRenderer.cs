...
/// Host apps pass their post/comment surface as plain render data and supply
/// the bundled font and default profile image bytes through
/// PostImageRendererResources; saving the resulting bytes (gallery, file
/// picker, temp file) stays the host app's job.
/// </summary>
public static class PostImageRenderer
{
    ...
    public static async Task<byte[]> RenderAsync(IEnumerable<BaseContent> contents, PostImageRendererResources resources = null, PostRenderHeader header = null, IEnumerable<CommentRenderData> comments = null, bool excludeMediaExceptFirst = false, SharedPostRenderData sharedPost = null) => await Task.Run(async () =>
    {
        ...
        var (regularTypeface, boldTypeface) = await GetTypefacesAsync(resources?.FontBytes);
    ...
    private static async Task<RenderBlock> BuildHeaderBlockAsync(string profileImageUrl, string nickname, string timestampText, byte[] defaultProfileImageBytes, RenderStyle style)
    {
        var hasProfile = profileImageUrl != null;
        var image = hasProfile ? await DownloadProfileImageOrDefaultAsync(profileImageUrl, defaultProfileImageBytes) : null;
    ...
    private static async Task<RenderBlock> BuildCommentBlockAsync(CommentRenderData comment, float contentWidth, byte[] defaultProfileImageBytes, RenderStyle style, bool excludeMediaExceptFirst = false)
    {
        var image = await DownloadProfileImageOrDefaultAsync(comment.ProfileImageUrl, defaultProfileImageBytes);
        var hasProfile = image != null;

        var columnWidth = contentWidth - CommentProfileSize - CommentColumnSpacing;
        var innerBlocks = await BuildBlocksAsync(comment.Contents ?? [], columnWidth, CommentMediaMaxWidth, style, excludeMediaExceptFirst);
        var innerHeight = innerBlocks.Count > 0 ? innerBlocks.Sum(block => block.Height) + ContentSpacing * (innerBlocks.Count - 1) : 0;
    ...
    private static async Task<List<RenderBlock>> BuildCommentBlocksAsync(IEnumerable<CommentRenderData> comments, float contentWidth, byte[] defaultProfileImageBytes, RenderStyle style, bool excludeMediaExceptFirst = false)
    {
        var blocks = new List<RenderBlock>();
        ...
        {
            var block = await BuildCommentBlockAsync(comment, contentWidth, defaultProfileImageBytes, style, excludeMediaExceptFirst);
            if (block != null) blocks.Add(block);
        }
        return blocks;
    }

    // Downloads the profile image, falling back to the host-supplied default profile
    // image bytes when the URL is missing or the download fails.
    private static async Task<SKImage> DownloadProfileImageOrDefaultAsync(string profileImageUrl, byte[] defaultProfileImageBytes)
    {
        ...
        try { return SKImage.FromEncodedData(defaultProfileImageBytes); }
        catch { return null; }
    }
    ...
    private static async Task<RenderBlock> BuildStickerBlockAsync(StickerContent stickerContent, RenderStyle style)
    {
        ...
        var isKakaoEmoticon = mediaId != null && mediaId.StartsWith(KakaoEmoticonUrlPrefix, StringComparison.Ordinal);
        var url = Uri.IsWellFormedUriString(mediaId, UriKind.Absolute) ? mediaId : CommonUtils.GenerateMediaUri(mediaId);
        var image = await DownloadImageAsync(url, isKakaoEmoticon ? KakaoStoryReferer : null);
    ...
    private static async Task<RenderBlock> BuildMediaBlockAsync(MediaContent mediaContent, float contentWidth, float maxMediaWidth, RenderStyle style)
    {
        ...
        var image = mediaId != null ? await DownloadImageAsync(Uri.IsWellFormedUriString(mediaId, UriKind.Absolute) ? mediaId : CommonUtils.GenerateMediaUri(mediaId)) : null;
    ...
    private static async Task<(SKTypeface Regular, SKTypeface Bold)> GetTypefacesAsync(byte[] fontBytes)
    {
