...
public partial class PollContentViewModel : ObservableObject, IContentViewModel
{
    ...
    private void UpdateOptions()
    {
        Options = PollContent.Options
            .Select((option, index) => new PollOptionViewModel(option, index, PollContent.MyVotedOptionIndices?.Contains(index) ?? false, this))
            .ToList();
    ...
    public async Task VoteAsync(int optionIndex)
    {
        ...
        if (PollContent.AllowMultipleSelection)
        {
            ...
            if (selectedIndices.Contains(optionIndex)) selectedIndices.Remove(optionIndex);
            else selectedIndices.Add(optionIndex);
        ...
        if (result.IsSuccess)
        {
            ...
            var updatedPollContent = result.Value.Contents.OfType<PollContent>().FirstOrDefault(pollContent => pollContent.PollId == PollId);
    ...
    {
        // TODO: Navigate to PollResultsPage (migrated in a later phase).
        await App.DisplayAlertAsync("안내", "투표 결과 페이지는 아직 지원되지 않습니다.", Constants.PromptOk);
    }
}
