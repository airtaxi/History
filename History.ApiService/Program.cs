...
builder.Services.AddCors(options =>
{
    ...
    {
        policy.WithOrigins("http://localhost:5173", "https://history.cenox.io", "https://historyweb.cc")
        .AllowAnyHeader()
        .AllowAnyMethod();
    });
});
...
builder.Services.Configure<WnsOptions>(builder.Configuration.GetSection(WnsOptions.SectionName));
builder.Services.AddSingleton<WnsAccessTokenCache>();
builder.Services.AddHttpClient(WnsPushNotificationProvider.HttpClientName, client => client.Timeout = TimeSpan.FromSeconds(30));
builder.Services.AddScoped<IPushNotificationProvider, FirebasePushNotificationProvider>();
builder.Services.AddScoped<IPushNotificationProvider, WnsPushNotificationProvider>();
